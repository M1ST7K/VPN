...
class PerAppProxyActivity : BaseActivity() {
    ...
    private fun bindHotfoxChrome() {
        findViewById<View>(R.id.btn_header_back)?.apply {
            visibility = View.VISIBLE
            setOnClickListener { finish() }
        }
        val include = findViewById<TextView>(R.id.btn_segment_include)
        val exclude = findViewById<TextView>(R.id.btn_segment_exclude)
        include?.setOnClickListener {
            if (binding.switchBypassApps.isChecked) {
                binding.switchBypassApps.isChecked = false
            }
        }
        exclude?.setOnClickListener {
            if (!binding.switchBypassApps.isChecked) {
                binding.switchBypassApps.isChecked = true
            }
        }
        findViewById<EditText>(R.id.et_search)?.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                filterProxyApp(s?.toString().orEmpty())
            }
        })
        findViewById<View>(R.id.row_custom_rules)?.setOnClickListener {
            startActivity(Intent(this, RoutingSettingActivity::class.java))
        }
        findViewById<View>(R.id.btn_save_bypass)?.setOnClickListener {
            allowPerAppProxy()
    ...
    private fun renderSegments() {
        val include = findViewById<TextView>(R.id.btn_segment_include) ?: return
        val exclude = findViewById<TextView>(R.id.btn_segment_exclude) ?: return
        val bypass = binding.switchBypassApps.isChecked
        val cream = ContextCompat.getColor(this, R.color.hf_asset_cream)
        val muted = ContextCompat.getColor(this, R.color.hf_asset_muted)
        include.setTextColor(if (bypass) muted else cream)
        exclude.setTextColor(if (bypass) cream else muted)
        include.setBackgroundResource(if (bypass) 0 else R.drawable.hf_surface_segment_selected)
        exclude.setBackgroundResource(if (bypass) R.drawable.hf_surface_segment_selected else 0)
    }

    private fun renderSelectionCount() {
        findViewById<TextView>(R.id.tv_selected_count)?.text = viewModel.getAll().size.toString()
        val rules = runCatching { com.v2ray.ang.vpn.HotfoxRoutingStore.load().rules.size }.getOrDefault(0)
        findViewById<TextView>(R.id.tv_custom_rules_count)?.text = rules.toString()
    }
