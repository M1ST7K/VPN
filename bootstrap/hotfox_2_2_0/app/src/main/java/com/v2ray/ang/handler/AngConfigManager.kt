...
object AngConfigManager {
    ...
    private fun findMatchedProfileKey(keyToProfile: Map<String, ProfileItem>, target: ProfileItem?): String? {
        ...
        if (target == null) {
            return HotfoxServerSelection.matchImportedKey(keyToProfile.keys, null)
        }
    ...
    private fun parseConfigViaSub(server: String?, subid: String, append: Boolean): Int {
        LogUtil.i(AppConfig.TAG, "Subscription format detected: ${SubscriptionFormatDetector.detect(server)}")
        var count = parseBatchConfig(Utils.decode(server), subid, append)
        if (count <= 0) {
            count = parseBatchConfig(server, subid, append)
        }
