...
class CoreTestService : Service() {
    ...
    @Volatile
    private var activeGuids: List<String> = emptyList()
    ...
    override fun onDestroy() {
        ...
        activeWorkers.clear()
        HotfoxServerSelection.cancelProbeCycle(activeGuids)
    ...
    private fun handleMeasureStart(message: TestServiceMessage, startId: Int) {
        ...
        if (guidsList.isNotEmpty()) {
            activeGuids = guidsList
            val generation = if (HotfoxServerSelection.health.generation > 0L) {
                HotfoxServerSelection.health.generation
            } else {
                HotfoxServerSelection.beginProbeCycle(guidsList)
            }
            lateinit var worker: RealPingWorkerService
            worker = RealPingWorkerService(
                context = this,
                guids = guidsList,
                onEvent = { event -> handleWorkerEvent(event, generation) { activeWorkers.remove(worker) } }
            )
    ...
    private fun handleWorkerEvent(event: RealPingEvent, generation: Long, onWorkerDone: () -> Unit) {
        when (event) {
            ...
            is RealPingEvent.Result -> {
                ...
                HotfoxServerSelection.ingestProbeResult(event.guid, event.delayMillis, generation)
