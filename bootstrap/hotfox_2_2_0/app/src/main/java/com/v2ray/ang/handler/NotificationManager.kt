...
object NotificationManager {
    ...
    fun showNotification(currentConfig: ProfileItem?) {
        ...
        mBuilder = NotificationCompat.Builder(service, channelId)
            .setSmallIcon(R.drawable.ic_hotfox_notification)
    ...
    private fun updateNotification(contentText: String?, proxyTraffic: Long, directTraffic: Long) {
        if (mBuilder != null) {
            if (proxyTraffic < NOTIFICATION_ICON_THRESHOLD && directTraffic < NOTIFICATION_ICON_THRESHOLD) {
                mBuilder?.setSmallIcon(R.drawable.ic_hotfox_notification)
    ...
    private fun stringRes(context: Context?, name: String, fallback: Int): String {
        if (context == null) return ""
        val id = context.resources.getIdentifier(name, "string", context.packageName)
        return if (id != 0) context.getString(id) else context.getString(fallback)
    }
}
