#!/usr/bin/env python3
"""Build side-by-side, 50% overlay and absdiff for 18 V5.1 screens."""
...
ACT = ROOT / "verification" / "ui_rebuild" / "actual_v5_1"
CMP = ROOT / "verification" / "ui_rebuild" / "compare_v5_1"
CSV_PATH = ROOT / "verification" / "ui_rebuild" / "HOTFOX_PIXEL_DIFF_RESULTS_V5_1.csv"
SIZE = (1080, 2400)
...
def main() -> int:
    ...
    for i in range(1, 19):
        ...
        if ref_p.is_file() and act_p.is_file():
            ...
            draw.text((SIZE[0] + 48, 12), f"{sid} actual emulator V5.1", fill=(243, 237, 233))
