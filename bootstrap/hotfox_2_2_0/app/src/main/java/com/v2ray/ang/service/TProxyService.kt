...
//        LogUtil.i(AppConfig.TAG, "Config file created: ${configFile.absolutePath}")
        LogUtil.d(AppConfig.TAG, "Tunnel configuration prepared")
...
//            LogUtil.i(AppConfig.TAG, "TProxyStartService...")
    ...
    private fun buildConfig(): String {
        ...
        return buildString {
            ...
            // VpnService captures IPv6 in both modes to prevent leaks. HEV therefore always
            // receives an IPv6 address; Xray decides whether IPv6 is proxied or blackholed.
    ...
    override fun getStats(): LongArray? = try {
        TProxyGetStats()?.takeIf { it.size == 4 }
    } catch (_: Exception) {
        null
    }
