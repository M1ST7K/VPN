#!/usr/bin/env python3
"""Capture HotFox presentation states via the debug screenshot harness (V5.1)."""
...
SMOKE_IDS = ("01", "03", "04", "05", "10", "12", "15", "17")
...
def dismiss_system_anr(max_tries: int = 10) -> None:
    for _ in range(max_tries):
        ...
        time.sleep(1.5)
    # Last attempt: still tap Wait in case dumpsys lagged.
    adb("shell", "input", "tap", "300", "1320", check=False)
    time.sleep(2.0)
...
def main() -> int:
    ...
    out_dir = ROOT / "verification" / "ui_rebuild" / (
        "actual_v5_1_smoke" if args.mode == "smoke" else "actual_v5_1"
    )
