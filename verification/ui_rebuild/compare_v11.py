#!/usr/bin/env python3
"""Build side-by-side, 50% overlay and absdiff for 18 V11 screens."""
...
ACT = ROOT / "verification" / "ui_rebuild" / "actual_v11"
CMP = ROOT / "verification" / "ui_rebuild" / "compare_v11"
CSV_PATH = ROOT / "verification" / "ui_rebuild" / "HOTFOX_PIXEL_DIFF_RESULTS_V11.csv"
SIZE = (1080, 2400)
...
def main() -> int:
    ...
    for i in range(1, 19):
        ...
        if ref_p.is_file() and act_p.is_file():
            ...
            draw.text((SIZE[0] + 48, 12), f"{sid} actual emulator V11 ru-RU", fill=(243, 237, 233))
            sbs.save(CMP / "side_by_side" / f"{sid}.png")
            Image.blend(ref, act, 0.5).save(CMP / "overlay" / f"{sid}.png")
            ImageChops.difference(ref, act).save(CMP / "diff" / f"{sid}.png")
    ...
    if captured == 18:
        ...
        cols, rows_n = 6, 3
        sheet = Image.new("RGB", (cols * thumb_w, rows_n * thumb_h), (13, 12, 18))
        for i in range(18):
            sid = f"{i + 1:02d}"
            im = load(ACT / f"{sid}.png").resize((thumb_w, thumb_h), Image.Resampling.LANCZOS)
            x = (i % cols) * thumb_w
            y = (i // cols) * thumb_h
            sheet.paste(im, (x, y))
        contact = ROOT / "verification" / "ui_rebuild" / "contact_v11.png"
        sheet.save(contact)
