#!/usr/bin/env python3
"""Capture HotFox presentation states via the debug screenshot harness (V9, ru-RU)."""
...
LOCALE_PROOF = ROOT / "verification" / "ui_rebuild" / "V9_LOCALE_PROOF.txt"
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default="",
        help="output directory name under verification/ui_rebuild, default actual_v9",
    )
    args = parser.parse_args()
    out_name = args.out or ("actual_v9_smoke" if args.mode == "smoke" and not args.screens else "actual_v9")
