...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun updateCache() {
        ...
        for (guid in serverList) {
            ...
            if (favoriteOnly && !MmkvManager.isServerFavorite(guid)) continue
            if (kw.isEmpty()) {
            ...
            val presented = HotfoxServerPresentation.fromRemark(remarks)
            if (remarks.matchesPattern(searchRegex, kw)
                ...
                || presented.country.orEmpty().matchesPattern(searchRegex, kw)
                || presented.title.matchesPattern(searchRegex, kw)
            ) {
                serversCache.add(ServersCache(guid, profile))
            }
        }
    ...
    fun getSubscriptions(context: Context): List<GroupMapItem> {
        ...
        val realSubscriptions = subscriptions.filterNot { it.guid == AppConfig.DEFAULT_SUBSCRIPTION_ID }

        // HOTFOX home is subscription-first. The internal Default bucket is only storage for
        // manually imported/legacy profiles and must not appear next to a real subscription.
        // This removes the confusing "Default | Sub" two-tab UI.
        val visibleSubscriptions = if (realSubscriptions.isNotEmpty()) realSubscriptions else subscriptions

        if (visibleSubscriptions.none { it.guid == subscriptionId }) {
            subscriptionId = visibleSubscriptions.firstOrNull()?.guid.orEmpty()
            MmkvManager.encodeSettings(AppConfig.CACHE_SUBSCRIPTION_ID, subscriptionId)
        }
        ...
        visibleSubscriptions.forEach { sub ->
            groups.add(
                GroupMapItem(
                    id = sub.guid,
                    remarks = if (sub.guid == AppConfig.DEFAULT_SUBSCRIPTION_ID) {
                        context.getString(R.string.hotfox_manual_servers)
                    } else {
                        sub.subscription.remarks.ifBlank { context.getString(R.string.hotfox_subscription) }
                    }
                )
            )
        }
    ...
    private fun sortByTestResultsForSub(subId: String) {
        data class ServerDelay(var guid: String, var favorite: Boolean, var testDelayMillis: Long)

        val serverDelays = mutableListOf<ServerDelay>()
        val serverListToSort = MmkvManager.decodeServerList(subId)

        serverListToSort.forEach { key ->
            val affiliation = MmkvManager.decodeServerAffiliationInfo(key)
            val delay = affiliation?.testDelayMillis ?: 0L
            serverDelays.add(ServerDelay(key, affiliation?.favorite == true, if (delay <= 0L) 999999 else delay))
        }
        serverDelays.sortWith(compareByDescending<ServerDelay> { it.favorite }.thenBy { it.testDelayMillis })
    ...
    val tunnelHealth = androidx.lifecycle.MutableLiveData<String>()

    private val mMsgReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                AppConfig.MSG_HOTFOX_HEALTH -> { tunnelHealth.value = intent.getStringExtra("content") }
                AppConfig.MSG_HOTFOX_TRAFFIC -> {
                    val parts = intent.getStringExtra("content").orEmpty().split(':')
                    val tx = parts.getOrNull(0)?.toLongOrNull() ?: 0L
                    val rx = parts.getOrNull(1)?.toLongOrNull() ?: 0L
                    tunnelTraffic.value = tx to rx
                }
