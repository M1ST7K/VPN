...
object HttpUtil {
    ...
    data class SubscriptionHttpResponse(
        val content: String,
    ...
    fun getUrlContentWithUserAgent(request: UrlContentRequest): String {
        return getSubscriptionResponse(request).content
    }

    /** Fetches a subscription body together with its quota header without persisting the URL. */
    @Throws(IOException::class)
    fun getSubscriptionResponse(request: UrlContentRequest): SubscriptionHttpResponse {
        ...
        while (redirects++ < maxRedirects) {
            ...
            if (!Utils.isValidSubUrl(currentUrl)) {
            ...
            val finalUserAgent = if (request.userAgent.isNullOrBlank()) {
                "HOTFOX-VPN/${BuildConfig.VERSION_NAME}"
            } else {
                request.userAgent
            }
            val requestBuilder = Request.Builder()
                ...
                when {
                    ...
                    response.isSuccessful -> {
                        ...
                        return SubscriptionHttpResponse(
                            content = content,
                            userInfoHeader = SubscriptionUserInfoParser.collectHeaders { name ->
