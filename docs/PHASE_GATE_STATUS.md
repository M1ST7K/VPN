...
Repository-side 3.1 work: `HotfoxEngineFacade`, signed control plane, capacity-aware AUTO, generated device registry, API compatibility, offline/outage policy, privacy-safe telemetry, 2.x migration, remote-flag denylist for fake CONNECTED / DNS / IPv6 weakening. Round 20 P1 (INCLUDE/EXCLUDE Xray vs APP precedence) is fixed on `844918b`; push CI `34475599536` SUCCESS. Round 21 did not review that head (PR-lifetime review cap on `main`).
